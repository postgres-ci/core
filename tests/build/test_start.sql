create or replace function build.test_start() returns void as $$
  declare 
        _project_id       int;
            ...
            IF assert.not_null(_project_id) THEN 
                ...
                IF assert.not_null(_commit_id) THEN 
                    ...
                    PERFORM assert.exception(
                        $sql$ SELECT build.start($sql$ || _build_id || $sql$) $sql$, 
                        exception_message  := 'NOT_FOUND',
                        exception_sqlstate := 'P0002'
                    );
                        
                    IF build.accept(_build_id) THEN 

                        PERFORM 
                            assert.equal('master',    branch),
                            assert.equal(_commit_sha, revision),
                            assert.equal((SELECT repository_url FROM postgres_ci.projects WHERE project_id = _project_id), repository_url)
                        FROM build.start(_build_id);
