create or replace function hook.test_github_push() returns void as $$
    declare 
        _project_id       int;
    ...
    begin 
        ...
        IF assert.not_null(_project_owner_id) THEN 
            ...
            IF assert.not_null(_project_id) THEN 
                ...
                _github_name = (SELECT github_name FROM postgres_ci.projects WHERE project_id = _project_id);

                IF assert.equal(3, (SELECT COUNT(*) FROM (SELECT commit_id FROM hook.github_push(_github_name, 'master', _push)) _ )::int) THEN 
